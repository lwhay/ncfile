...
public class BTreeRecord {
    ...
    public void create() {
        if (dbFile.exists()) {
            ...
            db = DBMaker.newFileDB(dbFile).make();
            mTree = db.createTreeMap(name).keySerializer(transSer(schema, keyFields)).valueSerializer(Serializer.STRING)
                    .make();
        }
    }
    ...
    public Object get(Record record, int[] fields) {
        assert (isBtree);
        Object key = transKey(record, fields);
    ...
    public Record getRecord(Record record, int[] fields) {
        Object v = get(record, fields);
        if (v != null) {
            return NestManager.readKeyRecord(valueSchema, v.toString());
        } else {
            return null;
    ...
    public static BTreeKeySerializer transSer(Schema s, int[] fields) {
        ...
        switch (fields.length) {
            ...
            case 6:
                return new Tuple6KeySerializer(null, null, null, null, null, sers[0], sers[1], sers[2], sers[3],
                        sers[4], sers[5]);
            default:
    ...
    public Record transValue(Record record) {
        ...
        if (valueFields != null) {
            ...
            for (int i = 0; i < keyFields.length; i++) {
                res.put(i, record.get(keyFields[i]));
            }
        }
        return res;
    }
