...
public class DataFileWriter<D> implements Closeable, Flushable {
    ...
    private void assertOpen() {
        if (!isOpen)
            throw new NeciRuntimeException("not open");
    }

    private void assertNotOpen() {
        if (isOpen)
            throw new NeciRuntimeException("already open");
    }
     ...
     * @param out
     *            positioned at the end of the existing file.
     */
    ...
    public DataFileWriter<D> setMeta(String key, byte[] value) {
        if (isReservedMeta(key)) {
            throw new NeciRuntimeException("Cannot set reserved meta key: " + key);
        }
